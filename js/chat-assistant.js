...
(function () {
  ...
  async function executeHealthcareSearch(lat, lng, radiusKm, category) {
    ...
    if (window.PlacesHealthService && window.PlacesHealthService.fetchNearbyFacilities) {
      ...
      // Direct /api/hospitals or live Nominatim POI query if PlacesHealthService not on page
      try {
        const rRes = await fetch(`/api/hospitals?lat=${lat}&lng=${lng}&radius=${radiusKm || 10}&category=${encodeURIComponent(category || 'All')}`);
        if (rRes.ok) {
          const rData = await rRes.json();
          if (rData && rData.success && Array.isArray(rData.hospitals)) {
            facilities = rData.hospitals;
          }
        }
      } catch (e) {}
